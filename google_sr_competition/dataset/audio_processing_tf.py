...
def tf_random_add_noise_transform_snr(wave, noise_limit=0.2, u=0.5):

    #add by SNR (signal to noise ratio)
    if random.random() < u:
        ...
        else: #wave_length == noise_length
            pass

        alpha  = np.random.random() * noise_limit
        noise2 = np.sum(noise**2)
        wave2  = np.sum(wave**2 )
        snr    = (wave2 / noise2)**0.5
        wave   = np.clip(alpha * snr * noise + wave, -1, 1)
